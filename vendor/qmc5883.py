...
class QMC5883:
    def __init__(self, scl, sda, i2c_id, slvAddr=0x30):
        ...
        self.i2c_writereg(
            0x0A, b"\xc9"
        )  # C:8*8over sample, 9:(100Hz ,normal)/D(200Hz,normal)
        # find suitable range
        self.status = b"\x02"
    ...
    def measure(self):
        ...
        x = mag[0] + (mag[1] << 8)
        xyz[0] = (
            x if (x & 0x8000 == 0) else x - 65536
        )  # complementry to ordinary,65536=2**16
        y = mag[2] + (mag[3] << 8)
